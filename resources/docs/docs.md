...
The `home-routes` handler in the `crux-luminus.routes.home` namespace
...
The `home-page` function will in turn call the `crux-luminus.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `crux-luminus.handler` namespace:
...
Request middleware functions are located under the `crux-luminus.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `crux-luminus.dev-middleware` namespace found in
...
* Let `mount` know to start the Crux node by `require`-ing `crux-luminus.db.core` in some other namespace.
